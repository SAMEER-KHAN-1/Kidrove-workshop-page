...
function App() {
  ...
  // Wake the free-tier backend on load so it's ready by the time
  // someone submits the form (otherwise the first request can stall).
  useEffect(() => {
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-900/96 backdrop-blur border-b border-white/10">
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
          <a
            ...
            className="flex items-center gap-2.5 text-white font-bold text-lg shrink-0"
          >
          ...
          <div className="hidden md:flex items-center gap-5 text-sm">
            ...
            <a
              href="#register"
              onClick={(e) => { e.preventDefault(); goTo('register'); }}
